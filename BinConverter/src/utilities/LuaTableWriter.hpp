...
namespace BinConverter {
	...
	template <class T>
	concept Writable = requires(std::ostream & os, T obj)
	{
		os << obj;
	};
	...
	public:
		...
		*/
		...
		template <Writable TValue> LuaWriter& WriteValue(TValue value, bool indent = false);
		template <Writable TValue> LuaWriter& WriteWords(TValue value, bool newLine = false, bool indent = false);
		template <Writable TValue> LuaWriter& WriteUnquotedValue(TValue value, bool newLine = false);
		template <Writable TKey, Writable TValue> LuaWriter& WriteQuotedKeyAndValue(TKey key, TValue value, bool newLine = true);
		template <Writable TKey, Writable TValue> LuaWriter& WriteKeyAndValue(TKey key, TValue value, bool newLine = true);
		template <Writable TKey, Writable TValue> LuaWriter& WriteKeyAndQuotedValue(TKey, TValue, bool newLine = true);

		template <Writable TTable> LuaWriter& StartBracketedTable(TTable name, bool newLine = true, bool sameLine = false);
	};
