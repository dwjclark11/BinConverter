...
void ImguiExt::End(WINDOWING::Window& window)
{
	...
	if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
	{
		...
		SDL_GL_MakeCurrent(window.GetWindow().get(), backup_current_context);
	}
}
...
void ImguiExt::MainMenu()
{
	...
	if (static auto bFirstTime = true; bFirstTime) [[unlikely]]
	{
		...
		const auto leftNodeId = ImGui::DockBuilderSplitNode(centerNodeId, ImGuiDir_Left, 0.3f, nullptr, &centerNodeId);
		const auto bottomNodeId =
			ImGui::DockBuilderSplitNode(centerNodeId, ImGuiDir_Down, 0.3f, nullptr, &centerNodeId);

		ImGui::DockBuilderDockWindow("Choices Display", leftNodeId);
		ImGui::DockBuilderDockWindow("Log", bottomNodeId);
