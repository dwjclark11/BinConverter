...
namespace BinConverter {
    ...
    {
        std::string indent(m_IndentSize, ' '); // Create the indent based on the desired size
    ...
    LuaWriter::LuaWriter(const std::string& filename, int indentSize)
        : m_CurrentFile{ }, m_Indent{ 0 }, m_IndentSize{ indentSize }, m_bValueWritten{ false }
        , m_bNewLine{ false }, m_bDocStarted{ false }
    {
